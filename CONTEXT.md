...
| Area | Current state |
| --- | --- |
| Frontend | React + TypeScript + Vite. `/` is a cinematic, scroll-driven PRISM project overview with an SVG/CSS learning orbit, semantic narrative sections, sticky anchor navigation, responsive re-composition, and reduced-motion support. The auth route now uses a short, composited transform-only curtain instead of cloning and tearing the full overview DOM. Catalog, subject paths, tutor, progress, and teacher dashboard routes remain wired. |
